...
class YearBy:
    ...
    def display_title(self):
        Label(self.screen, text='Year By Year Calculator', fg=settings.WHITE, bg=settings.BLACK,
    ...
    def output_ending_capital(self):
        ...
        Label(self.screen, text='\nEnding Capital:', fg=settings.WHITE,
              bg=settings.BLACK, font=settings.menu_font).grid(row=7, column=0, sticky=NSEW)
        self.ending_capital_disp = Text(self.screen, width=20, height=1, wrap=WORD,
                                        background=settings.WHITE, font=settings.entry_font)
        self.ending_capital_disp.grid(row=7, column=1, sticky=NSEW)

    def display_sequence_buttons(self):
        # Next Year, New Sequence
        Button(self.screen, text='New Sequence', width=14,
               command=self.calculate_ending_capital).grid(row=6, column=0, sticky=N, pady=10)
        Button(self.screen, text='Add Sequence', width=14,
               command=self.calculate_ending_capital).grid(row=6, column=1, sticky=N, pady=10)
